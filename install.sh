...
create_subcmd "architecture-design" "Generate 方式設計書 (Architecture Design)" "@req"
...
create_subcmd "db-design" "Generate データベース設計書 (DB Design)" "@bd"
create_subcmd "screen-design" "Generate 画面設計書 (Screen Design)" "@bd"
create_subcmd "interface-spec" "Generate IF仕様書 (Interface Spec)" "@bd"
create_subcmd "report-design" "Generate 帳票設計書 (Report/Form Design)" "@bd"
create_subcmd "batch-design" "Generate バッチ処理設計書 (Batch Design)" "@bd"
...
create_subcmd "test-result-report" "Generate テスト結果報告書 (Test Result Report)" "@test-specs"
create_subcmd "test-evidence" "Generate テストエビデンス (Test Evidence)" "@test-spec"
# ── Management commands ──
create_subcmd "meeting-minutes" "Generate 議事録 (Meeting Minutes)" "@notes"
create_subcmd "decision-record" "Generate 設計判断記録 (Architecture Decision Record)" "@notes"
...
create_subcmd "dashboard" "Open analytics dashboard (quality metrics, traceability, risk)" ""
