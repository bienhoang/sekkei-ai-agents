...
export const GENERATION_INSTRUCTIONS: Record<DocType, string> = {
  ...
  nfr: [
    ...
    "",
    "### IPA Grade Reference (Section 2 of template)",
    "The template includes IPA grade tables (A/B/C/D) for 可用性, 性能, セキュリティ.",
    "Select ONE grade per category based on project context:",
    "- Grade A: mission-critical (金融基幹, 医療) — 99.999% uptime, MFA必須",
    "- Grade B: business-critical (EC, SaaS, 官公庁) — 99.99% uptime, SSO+MFA推奨",
    "- Grade C: standard (社内業務, BtoB) — 99.9% uptime, password+SSO",
    "- Grade D: low (情報系, 少人数) — 99% uptime, basic auth",
    "State selected grade in Section 1 概要, then use grade targets as basis for numeric 目標値.",
    "",
  ...
  "architecture-design": [
    "Generate a 方式設計書 (Architecture Design) from requirements + NFR.",
    "Follow IPA共通フレーム2013 方式設計 structure.",
    "5 sections: システム方式, 開発方式, 運用方式, ハードウェア・ネットワーク構成, 技術選定.",
    "ID format: ARCH-001. Each architecture decision references upstream REQ-xxx, NFR-xxx.",
    "システム方式: architecture patterns (monolith/microservice/serverless), deployment topology.",
    "開発方式: development methodology, branching strategy, CI/CD pipeline, coding standards.",
    "運用方式: monitoring, logging, backup, disaster recovery approach.",
    "HW/NW構成: server specs, network topology (Mermaid diagram), cloud region, AZ strategy.",
    "技術選定: technology stack rationale table: | 技術領域 | 選定技術 | 理由 | 代替案 |",
    "Cross-reference REQ-xxx, NFR-xxx from upstream. Reference F-xxx for scope context.",
  ].join("\n"),

  "test-result-report": [
    "Generate a テスト結果報告書 (Test Result Report) from test specs + evidence.",
    "This is a management summary — NOT individual test cases.",
    "7 sections: テスト概要, テスト実施結果サマリー, テストレベル別結果, 不具合サマリー, 品質判定, 残課題・リスク, 総合判定.",
    "ID format: TR-001. Each TR entry is a test-level result summary.",
    "テスト実施結果サマリー table: | テストレベル | 総件数 | 合格 | 不合格 | 未実施 | 合格率 |",
    "不具合サマリー table: | 重大度 | 件数 | 解決済 | 未解決 | 対応状況 |",
    "品質判定: state go/no-go recommendation with quantitative criteria (e.g., 合格率95%以上).",
    "Cross-reference UT-xxx, IT-xxx, ST-xxx, UAT-xxx, TP-xxx from upstream test specs.",
    "Include テスト実施日, 実施者, エビデンス参照 per test level.",
  ].join("\n"),

  "db-design": [
    "Generate a データベース設計書 (Database Design) from basic-design + requirements.",
    "This is a STANDALONE database design — not the embedded DB sections in basic-design.",
    "8 sections: DB設計方針, ER図, テーブル詳細定義, インデックス設計, パーティション設計, データ移行設計, バックアップ・リカバリ, 命名規約.",
    "ID format: DB-001. Each DB entry is a design decision (index, partition, replication).",
    "ER図: Mermaid erDiagram referencing TBL-xxx from basic-design.",
    "テーブル詳細定義: expand each TBL-xxx with full column list, constraints, FK references.",
    "インデックス設計 table: | DB-ID | テーブル名 | インデックス名 | カラム | 種別 (B-tree/Hash/GIN) | 目的 |",
    "パーティション設計: partition strategy (range/hash/list) per large table with volume estimates.",
    "データ移行設計: data conversion rules from source to target schema. Reference MIG-xxx if migration-design exists.",
    "Cross-reference TBL-xxx, REQ-xxx, NFR-xxx from upstream.",
  ].join("\n"),

  "report-design": [
    "Generate a 帳票設計書 (Report/Form Design) from basic-design.",
    "Standalone expansion of basic-design Section 6 (帳票設計).",
    "6 sections: 帳票概要, 帳票一覧, 帳票レイアウト, 出力条件, 帳票データマッピング, 印刷・配信設計.",
    "Use existing RPT-xxx IDs from basic-design. Add new entries as RPT-NNN.",
    "帳票一覧 table: | RPT-ID | 帳票名 | 出力形式 (PDF/Excel/CSV) | 出力タイミング | 配信先 | 備考 |",
    "帳票レイアウト: describe layout per RPT-xxx (header, detail, footer, page break rules).",
    "出力条件: filtering/sorting/grouping criteria per report.",
    "Cross-reference F-xxx, SCR-xxx, TBL-xxx from upstream.",
  ].join("\n"),

  "batch-design": [
    "Generate a バッチ処理設計書 (Batch Processing Design) from basic-design + functions-list.",
    "Standalone expansion of basic-design Section 8 (ジョブスケジュール).",
    "7 sections: バッチ概要, ジョブ一覧, ジョブフロー図, ジョブ詳細設計, スケジュール設計, エラーハンドリング, 運用連携.",
    "ID format: BATCH-001. Each entry is a batch job definition.",
    "ジョブ一覧 table: | BATCH-ID | ジョブ名 | 実行タイミング | 依存ジョブ | リトライ回数 | タイムアウト | 備考 |",
    "ジョブフロー図: Mermaid flowchart showing job dependencies and execution order.",
    "スケジュール設計: cron expressions, maintenance windows, conflict avoidance.",
    "エラーハンドリング: retry policy, dead-letter handling, alert escalation per job.",
    "Cross-reference F-xxx (batch functions), TBL-xxx (data targets), OP-xxx (operation procedures).",
  ].join("\n"),
...
export const KEIGO_MAP: Record<DocType, KeigoLevel> = {
  ...
  nfr: "丁寧語",
  "project-plan": "丁寧語",
  "architecture-design": "丁寧語",
  "basic-design": "丁寧語",
  "security-design": "simple",
  "detail-design": "simple",
  "db-design": "simple",
  "report-design": "simple",
  "batch-design": "simple",
  "test-plan": "simple",
  "ut-spec": "simple",
  "it-spec": "simple",
